...
if [[ "x${cephadmin}" == 'xtrue' ]]; then
  ...
  sudo apt-get update
  sudo apt-get install --assume-yes ceph-deploy
fi
...
if [[ "x${cephadmin}" == 'xtrue' ]]; then
  ...
  sudo -u "${ceph_deploy_user}" -i -- bash -c "ssh-keygen -b 4096 -N '' -f '${ssh_identity_file}'"
  ...
  for node in "${ceph_nodes[@]}"; do
    sudo -u "${ceph_deploy_user}" -i -- bash -c "ssh-copy-id -i '${ssh_identity_file}' '${ceph_deploy_user}@${node}'"
  ...
  sudo -u "${ceph_deploy_user}" -i -- cat "${ssh_config}"
